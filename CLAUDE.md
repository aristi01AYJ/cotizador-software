...
- **Columnas en SharePoint: `FechaCierre` (Fecha y hora) y `PosibleCierre` (Una línea de texto) — creadas por el usuario en `cotizaciones_sw` el 22-sep-2026**, junto con `TasaEur` (ver fix de moneda abajo). Las 3 confirmadas creadas; no queda ninguna columna pendiente de las fases 1-3. Sin verificación aún con datos reales (este entorno no tiene login a SharePoint) — si algo no aparece, revisar consola (F12) primero.
